...
begin
  ...
  begin
    px := tmp^.Info.x;
    py := tmp^.Info.y;
    // «апрещаем проводить пр€мую под углом.
    try
...
// ѕроцедура превращает линии под углом в вертикальные или горизонтальные
// ¬ зависимости от угла наклона. (¬се линии в синтаксических диаграммах
// ƒолжны быть параллельны одной из осей.
...
{»де€ алгоритма: найти область линии, все точки которой иметь
либо координату x, либо координату y, равную старому значение
координаты перемещаемой точки. ѕри этом все точки области
должны идти подр€д и в области не должно содержатьс€ ни
одной точки, не соответствующих данному условию.
ѕосле нахождени€ области, нужно изменить координаты
...
begin
  ...
  begin
    ...
    begin
      BeginOfArea := nil; // ≈сли точка противоречит одному из перечисленных выше условий,
